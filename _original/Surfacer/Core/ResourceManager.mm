...
/*
		...
		typedef std::map< dual_file_resource_key, gl::GlslProgRef > shader_cache;
...
gl::GlslProgRef ResourceManager::getShader( const fs::path &shaderNameFragment )
{
	return getShader( 
...
gl::GlslProgRef ResourceManager::getShader( const fs::path &vertexFileNameFragment, const fs::path &fragFileNameFragment )
{
	...
	if ( !shader )
	{
		try 
		{
			shader = std::make_shared<gl::GlslProg>(gl::GlslProg( loadResource(fs::path(key.first)), loadResource(fs::path(key.second)) ));
			_shaders[key] = shader;
		}
...
gl::GlslProgRef ResourceManager::_findShader( const dual_file_resource_key &key ) const
{
