...
struct collision_info
{
	...
	Vec2 position( int i = 0 ) const { return v2(cpArbiterGetPoint(arbiter,i)); }
	Vec2 normal( int i = 0 ) const { return v2(cpArbiterGetNormal(arbiter,i)); }
	...
	Vec2 velocity() const { return v2(cpArbiterGetSurfaceVelocity(arbiter)); }
	...
	*/
	Vec2 relativeVelocityFrameA() const {
		return v2( cpBodyGetVel( bodyB ) - cpBodyGetVel( bodyA ));
	}

	/**
		return the velocity of the collision in B's reference frame
	*/
	Vec2 relativeVelocityFrameB() const {
		return v2( cpBodyGetVel( bodyA ) - cpBodyGetVel( bodyB ));
	}
	...
	*/
	Vec2 slideVelocity( int i = 0 ) const {
		cpVect p = cpArbiterGetPoint(arbiter,i);
		return v2(cpvsub(cpBodyGetVelAtWorldPoint( bodyA, p), cpBodyGetVelAtWorldPoint( bodyB, p))); 
	}
