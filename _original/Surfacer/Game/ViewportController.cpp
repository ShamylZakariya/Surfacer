	...
	if ( _trackingEnabled )
	{
		...
		if ( target )
		{
			if ( !_activeViewportFitSensors.empty() )
			{
				...
				lookAt( v2(cpBBCenter(_activeViewportFitSensorsCombinedAabb)), Zoom );				
			}
			else
			{
				lookAt( v2(cpBBCenter( target->aabb())), level->initializer().defaultZoom );	
			}
		}
	}
...
void ViewportController::setZoom( real z, const Vec2 &aboutScreen )
{
	Mat4 mv, imv;
...
void ViewportController::lookAt( const Vec2 &world, real zoom, const Vec2 &screen )
{
...
void ViewportController::_updateZeno( const time_state &time )
{
	...
	Vec2 panError = _pan - _viewport.pan();
