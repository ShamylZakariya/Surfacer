...
class ViewportController : public core::Behavior
{
	...
	public:
		...
		*/
		void lookAt( const Vec2 &world, real zoom, const Vec2 &screen );

		/**
			Shortcut to lookAt( const Vec2 &world, real zoom, const Vec2 &screen )
		*/
		void lookAt( const Vec2 &world, real zoom ) { lookAt( world, zoom, _viewport.viewportCenter() ); }
		
		/**
			Shortcut to lookAt( const Vec2 &world, real zoom, const Vec2 &screen )
			Causes the camera to center @a world in the screen
		*/
		void lookAt( const Vec2 &world ) { lookAt( world, _zoom, _viewport.viewportCenter() ); }
