...
void SensorTestScenario::setup()
{
	...
	if ( true )
	{
		...
		//playerInit.position = Vec2( 60,112 );
		
		// by the powerups
		//playerInit.position = Vec2( 100,86 );
	...
	if ( true )
	{
		...
		if ( false )
		{
			...
			sensorInit.position = Vec2(180,142);
			sensorInit.width = 20;
			sensorInit.height = 6;
			...
			sensorInit.position = Vec2(198,142);
			sensorInit.width = 20;
			sensorInit.height = 6;
			sensorInit.angle = toRadians( real(-15) );
			sensor = new ViewportFitSensor();
			...
			sensorInit.position = Vec2(190,172);
		...
		sensorInit.position = Vec2(160,143);
		...
		sensorInit.position = Vec2(150,141);
		sensorInit.width = sensorInit.height = 4;
		sensorInit.angle = toRadians( real(0) );
		sensorInit.targetType = SensorTargetType::TERRAIN;
		...
		sensorInit.position = Vec2(155,150);
		sensorInit.width = sensorInit.height = 4;
		sensorInit.angle = toRadians( real(0) );
		sensorInit.targetType = SensorTargetType::TERRAIN;
		...
		sensorInit.position = Vec2(220,150);
		...
		sensorInit.position = Vec2(200,150);
		sensorInit.width = sensorInit.height = 16;
		sensorInit.targetType = SensorTargetType::PLAYER;
		sensor = new Sensor();
		sensor->initialize( sensorInit );
	...
	if ( true )
	{
		...
		battery = new PowerUp();
		batteryInit.position = Vec2(50,114);
		batteryInit.type = PowerUpType::BATTERY;
		battery->initialize(batteryInit);
		level->addObject( battery );

		battery = new PowerUp();
		batteryInit.position = Vec2(47,116);
		battery->initialize(batteryInit);
		level->addObject( battery );

		medikit = new PowerUp();
		medikitInit.position = Vec2(44,118);
		medikitInit.type = PowerUpType::MEDIKIT;
		medikit->initialize(medikitInit);
		level->addObject( medikit );
	}	
...
bool SensorTestScenario::mouseMove( const app::MouseEvent &event, const Vec2 &delta )
{
	_mouseWorld = level()->camera().screenToWorld( event.getPos() );
	return false;
}

bool SensorTestScenario::mouseDrag( const app::MouseEvent &event, const Vec2 &delta )
{
	GameLevel *level = gameLevel();
