	...
	if ( false )
	{
		...
		if ( true )
		{
			...
			for ( int i = 0; i < 64; i++ )
			{
				...
				rtbInit.position = Vec2( Rand::randFloat(),Rand::randFloat() );
...
bool UIPlaygroundScenario::mouseMove( const app::MouseEvent &event, const Vec2 &delta )
{
	return false;
}

bool UIPlaygroundScenario::mouseDrag( const app::MouseEvent &event, const Vec2 &delta )
{
	return false;
}
	...
	if ( _background )
	{
		switch( event.getCode() )
		{
			case app::KeyEvent::KEY_UP:
				...
				_background->setAlignment( _background->alignment() + Vec2( 0,0.1 ));
			...
			case app::KeyEvent::KEY_LEFT:
				_background->setAlignment( _background->alignment() + Vec2( -0.1, 0 ));
				app::console() << "Background.alignment: " << _background->alignment() << std::endl;
				break;
