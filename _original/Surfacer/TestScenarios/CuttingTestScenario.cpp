...
namespace {
	...
	void drawLineSegment( const Vec2 &a, const Vec2 &b, real thickness )
	{
		util::line_segment segment(a, b);
		Vec2 right( segment.dir.y, -segment.dir.x );
		right *= thickness * 0.5;
...
void CuttingTestScenario::setup()
{
	...
	if ( PLAYER_ENABLED )
	{
		...
		playerInit.width = 1;
		playerInit.position = Vec2(3.5 * 64, 5.5 * 64 ) * LevelScale;
...
bool CuttingTestScenario::mouseMove( const app::MouseEvent &event, const Vec2 &delta )
{
	Viewport &camera( level()->camera());
...
bool CuttingTestScenario::mouseDrag( const app::MouseEvent &event, const Vec2 &delta )
{
	Viewport &camera( level()->camera());

	Vec2 screen = event.getPos();
	_mouseWorld = camera.screenToWorld( screen );

	Vec2 dp = screen - _mouseScreen;
	_mouseScreen = screen;
