...
namespace {
	...
	inline Vec2 snapToVoxel( const Vec2 &ordinal )
	{
		return Vec2( std::floor( ordinal.x + real(0.5) ) + real(0.5), std::floor( ordinal.y + real(0.5) ) + real(0.5));
	}
	...
	inline Vec2 invertY( const Vec2 &v )
	{
		return Vec2( v.x, real(1) - v.y );
	}
	...
	void add_greeble_particle( 
		...
		const Vec2 &Offset,
		const Vec2 &TotalOrdinalSizeReciprocal,
		bool isMask )
	{
		int a,b,c,d;
		...
		Vec2 origin( position + range( d, -radius * 0.5, radius * 0.5 ) * segment.dir ),
...
void Island::_createPerimeterGreebling( Vec2 const *ordinalToCentroidRelativeOffset )
{
	const real terrainScale = _store->scale();

	const Vec2 
		Offset = ordinalToCentroidRelativeOffset ? *ordinalToCentroidRelativeOffset : Vec2(0,0),
	...
	foreach( Vec2rVec &voxelPerimeter, _voxelPerimeters )
	{
		if ( voxelPerimeter.size() > 2 )
		{
			...
			for ( int i = 0, N = voxelPerimeter.size(); i < N; i++ )
			{
				...
				{
					Vec2 perimeterPoint = perimeterSegment.lrp(d),
					...
					{
						Vec2 localPointOnSegment = perimeterPoint + Offset;
