...
namespace {
	...
	#else
		inline real Area( const Vec2 &a, const Vec2 &b, const Vec2 &c )
		{
...
bool Island::triangulate( Vec2 const *ordinalToCentroidRelativeOffset )
{
...
bool Island::_triangulate( Vec2 const *ordinalToCentroidRelativeOffset )
{
	const Vec2 offset = ordinalToCentroidRelativeOffset ? *ordinalToCentroidRelativeOffset : Vec2(0,0);
	...
	std::vector< triangle > &triangulation )
{
	...
	const Vec2 TotalOrdinalSizeReciprocal( real(1) / real( _store->width() * terrainScale ), real(1) / real(_store->height() * terrainScale) );
				...
				*c = t->GetPoint(2);
				...
				if ( Area( *a, *b, *c ) > Epsilon )
				{
					...
					triangulation.push_back(triangle(
						Vec2( c->x, c->y ), InvertY((Vec2( c->x, c->y ) - offset ) * TotalOrdinalSizeReciprocal),
						Vec2( b->x, b->y ), InvertY((Vec2( b->x, b->y ) - offset ) * TotalOrdinalSizeReciprocal),
						Vec2( a->x, a->y ), InvertY((Vec2( a->x, a->y ) - offset ) * TotalOrdinalSizeReciprocal)
					));
				}
			}
