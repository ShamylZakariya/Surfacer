...
class CuttingBeam : public BeamWeapon 
{
	public:
		...
		struct beam_segment {
			Vec2 start, end, dir, right;
			real length;
			
			beam_segment(){}
			beam_segment( const Vec2 &s, const Vec2 &e ):
				start(s),
				end(e)
			{
				length = s.distance(e);
	...
	public:
		...
		virtual Vec2 beamOrigin() const { return _beamSegments.front().start; }
		virtual Vec2 beamEnd() const { return _beamSegments.front().end; }
...
class CuttingBeamRenderer : public core::DrawComponent
{
	...
	private:
		...
		struct vertex {
			...
			vertex(){}
			vertex( const Vec2 &p, const Vec4 &c ):
				position(p),
				color(c)
			{}
		};
		...
		void _renderCircle( const Vec2 &position, real radius, const Vec4 &color, vertex_vec::iterator &vertex ) const;
		void _renderLine( const Vec2 &start, const Vec2 &dir, real length, real width, const Vec4 &startColor, const Vec4 &endColor, vertex_vec::iterator &vertex ) const;
