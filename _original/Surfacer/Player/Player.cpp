...
void PlayerRenderer::ready()
{
	if ( !_svg )
	{
		...
		const Vec2 DocumentSize = _svg.documentSize();
...
Vec2 PlayerRenderer::barrelEndpointWorld()
{
...
void PlayerRenderer::_drawDebug( const render_state &state )
{
	...
	{
		const Vec2
 			Pos = player->position() + Vec2( 0, -player->initializer().height/2 ),
