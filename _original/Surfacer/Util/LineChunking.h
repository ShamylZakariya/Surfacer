...
namespace core { namespace util {
	...
	{
		Vec2 _origin;
		chunked_line_sort( const Vec2 &origin ):_origin(origin){}
	...
	inline void ChunkLine( const Vec2 &start, const Vec2 &end, const Vec2i chunkSize, Vec2rVec &points )
	{
		...
		if ( chunkSize.x > 0 )
		{
			if ( dir.x > Epsilon )
			{
				...
				for ( int chunkXIndex = firstXChunkIndex; chunkXIndex <= lastXChunkIndex; chunkXIndex++ )
				{
					...
					points.push_back( Vec2( xChunkX, slopeXB + xChunkY ) );
					chunkedX = true;
				}				
			}
		...
		if ( chunkSize.y > 0 )
		{
			if ( dir.y > Epsilon )
			{
				...
				for ( int chunkYIndex = firstYChunkIndex; chunkYIndex <= lastYChunkIndex; chunkYIndex++ )
				{
					...
					points.push_back( Vec2( slopeYB + yChunkX, yChunkY ));
					chunkedY = true;
				}
			}
