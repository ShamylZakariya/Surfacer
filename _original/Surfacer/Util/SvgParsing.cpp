...
namespace {
	...
	Mat4 parseTranslateTransform( const std::string svgTranslateTransform )
	{
		...
		*/
		...
		return Mat4::createTranslation( Vec3( tx, ty, 0 ));		
	}
	...
	Mat4 parseScaleTransform( const std::string svgScaleTransform )
	{
		...
		*/
		...
		return Mat4::createScale( Vec2( sx, sy ));
	...
	Mat4 parseRotateTransform( const std::string svgRotateTransform )
	{
		...
		*/
		...
		Mat4 rot = Mat4::createRotation( Vec3(0,0,1), values[0] );
		...
		else if ( values.size() == 3 )
		{
			...
			Mat4 
				t0 = Mat4::createTranslation( Vec3( tx, ty, 0 )),
				t1 = Mat4::createTranslation( Vec3( -tx, -ty, 0 ));
	...
	class PathParser
	{
		...
		protected:
			...
			void arcTo( real rx, real ry, real xAxisRotation, bool largeArcFlag, bool sweepFlag, Vec2 end, bool relative );		
			void close();
	...
	void PathParser::arcTo( real rx, real ry, real xAxisRotation, bool largeArcFlag, bool sweepFlag, Vec2 end, bool relative )
	{
		if ( relative )
		{
			end += _lastPoint;
