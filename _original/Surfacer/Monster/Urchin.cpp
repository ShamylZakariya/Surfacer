...
void Urchin::addedToLevel( Level *level )
{
	...
	if ( true )
	{
		...
		for ( int i = 0, N = 16; i < N; i++ )
		{
			...
			const Vec2 Dir( std::cos( Rads ), std::sin( Rads ));
									
			frondInit.length = _initializer.size * Rand::randFloat( 1.75, 2.25 );
			_whiskers->addFrond( frondInit, _body, v2(cpBodyGetPos(_body)) + _centerOfMassOffset + Dir * 0.1, Dir );
		}
	}	
}
...
void Urchin::update( const time_state &time )
{
	...
	_up = rotateCCW( dir );
	_position = v2( cpBodyGetPos( _body )) + _centerOfMassOffset;	
...
void UrchinRenderer::_drawGame( const render_state &state )
{
	...
	const Vec2 Scale = _svgScale * urchin->lifecycle();
