...
void FrondEngine::update( const core::time_state &state )
{
	...
	for( frond_state_vec::iterator frond(_fronds.begin()),end(_fronds.end()); frond != end; ++frond )
	{
		...
		frond->positions.push_back( 
			v2(cpBodyGetPos(frond->bodies.front() )) + 
			v2(cpBodyGetRot(frond->bodies.front())) * 
			-HalfSegLength );
		...
		for( cpBodyVec::const_iterator body(frond->bodies.begin()),bEnd(frond->bodies.end()); body != bEnd; ++body )
		{
			...
			frond->positions.push_back( v2(cpBodyGetPos( *body )) + v2(cpBodyGetRot(*body )) * HalfSegLength );
...
void FrondEngine::addFrond( const frond_init &frond, cpBody *attachToBody, const Vec2 &positionWorld, const Vec2 &dirWorld )
{
	assert( level() );
