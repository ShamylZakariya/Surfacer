...
Vec2 Tongue::position() const
{
	return v2(cpBodyGetPos(body()));
}

Vec2 Tongue::up() const
{
	return Vec2(0,1);
}

Vec2 Tongue::eyePosition() const
{
	return position();
}
...
// Tongue
void Tongue::attachTo( Monster *parent, cpBody *body, const Vec2 &positionWorld )
{
	assert( !attached() );
	assert( !this->parent() );
...
void Tongue::attach( const Vec2 &positionWorld )
{
	assert( !attached() );
	assert( !_segments.empty() );
...
void TongueRenderer::_updateTrianglulation( const core::render_state &state )
{
	...
	Vec2 dir = rotateCW( v2f( cpBodyGetRot( tongue->body() )) );
	
	while( splineVertex != splineEnd )
	{
		Vec2 right = rotateCW( dir );
		real	
