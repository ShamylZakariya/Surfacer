...
Vec2 MotileFluidMonster::position() const 
{ 
	return v2( cpBodyGetPos( _motileFluid->centralBody() ));
}

Vec2 MotileFluidMonster::up() const 
{ 
	return Vec2(0,1); 
}

Vec2 MotileFluidMonster::eyePosition() const
{
...
void MotileFluidMonsterController::_getMotionProbeRays( cpVect &leftOrigin, cpVect &leftEnd, cpVect &rightOrigin, cpVect &rightEnd )
{
	...
	const Vec2 center = motileFluidMonster->position();
