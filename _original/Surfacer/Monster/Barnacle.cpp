...
void Barnacle::addedToLevel( Level *level )
{
	...
	const Vec2
		TongueAttachmentPoint = _initializer.position + Vec2(0,-(BodyTriangleHeight+BodyCircleRadius));
	...
	if ( true )
	{
		...
		if ( true )
		{
			...
			for ( int i = 0, N = 16; i < N; i++ )
			{
				...
				_whiskers->addFrond( frondInit, _body, v2(cpBodyGetPos(_body)) + Dir * _initializer.size * 0.5, Dir );
			}
		}
	}
}
...
void Barnacle::update( const time_state &time )
{
	...
	if ( _deathAcidDispatchNeeded )
	{
		...
		for ( int i = 0, N = 16 * _initializer.size * _initializer.size; i < N; i++ )
		{
			((GameLevel*)level())->emitAcid( 
				...
				Vec2( (Rand::randBool() ? -1 : 1) * Rand::randFloat(2), 0 ) );
		}
	...
	Vec2 dir = v2( cpBodyGetRot( _body ));
	_up = rotateCCW( dir );
	_position = v2( cpBodyGetPos( _body ));	
...
void BarnacleRenderer::_drawBody( const render_state &state )
{
	...
	_bulb.setScale( Vec2(WidthPulse, HeightPulse) );
