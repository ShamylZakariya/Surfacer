...
void Tentacle::update( const time_state &time )
{
	...
	Vec2 playerPosition = shub->controller()->lastKnownPlayerPosition();
	...
	for( segmentvec::iterator segment(_segments.begin()),end(_segments.end()); segment!=end; ++segment )
	{
		...
		segment->position = v2(cpBodyLocal2World( segment->body, cpv(0,-segment->length/2)));
		...
		if ( segment->rotation )
		{
			...
			if ( aggression > Epsilon )
			{
				Vec2 
					...
					dirToPlayerLocal = normalize( v2(cpBodyWorld2Local( segment->body, cpv(playerPosition))));
		...
		++tentacle )
	{
		...
		if ( idx >= 0 )
		{
			...
			if ( TENTACLES_CAN_BE_SEVERED )
			{
				...
				{
					Vec2 a( shotTentacle->segments()[idx].position ),
	...
	if ( idx >= 0 && idx < tentacle->segments().size() )
	{
		...
		for ( int i = 0; i < 8; i++ )
		{
			((GameLevel*)level())->emitAcid( 
				segment.position, 
				(segment.dir * vel) + Vec2( Rand::randFloat(-0.25,0.25),Rand::randFloat(-0.25,0.25)));
		}
...
void ShubNiggurath::_emitGrub()
{
	...
	if ( !(controller() && controller()->playerVisible() && _aim( controller()->lastKnownPlayerPosition(), launchVel, dir, tries )))
	{
		...
		dir = normalize( Vec2( (Rand::randBool() ? -1:1) * Rand::randFloat(0.25, 0.50), 1 ));
...
bool ShubNiggurath::_aim( const Vec2 &targetPosition, real &linearVel, Vec2 &dir, int triesRemaining )
{
...
void TentacleRenderer::_drawGame( const render_state &state )
{
	...
	const GLsizei 
		...
		positionOffset = 0,
		texCoordOffset = sizeof( Vec2 ),
		heatOffset = sizeof( Vec2 ) + sizeof( Vec2 ),
		opacityOffset = sizeof( Vec2 ) + sizeof( Vec2 ) + sizeof( float );
	...
	_shader.uniform( "ColorTextureCoord", Vec2( 0, injuryLevel ) );
...
void TentacleRenderer::_drawDebug( const render_state &state )
{
	...
	foreach( const Tentacle::segment &segment, tentacle->segments() )
	{
		...
		if ( segment.joint )
		{
			gl::drawSolidCircle( v2( cpPivotJointGetAnchr2( segment.joint )), 1.1 * segment.width/2, 16 );
		}
		
		gl::color( 1,0,0,1 );
		gl::drawLine( Vec2(0,0), Vec2(unitVectorLength,0) );
		gl::color( 0,1,0,1 );
		gl::drawLine( Vec2(0,0), Vec2(0, unitVectorLength) );
...
void TentacleRenderer::_updateSplineReps()
{
		...
		++segment )
	{
		...
		else
		{
			if ( segment != tentacle->segments().begin() )
			{
				...
				spline.segmentVertices.push_back(v2(cpBodyLocal2World( previousSegment.body, cpv( 0, previousSegment.length/2 ))));
	...
	lastSplineRep.segmentVertices.push_back( v2(cpBodyLocal2World( lastSegment.body, cpv( 0, lastSegment.length/2 ))) );
...
namespace {
	...
	float segment_extents( const Vec2 &segStart, const Vec2 &segEnd, float halfWidth, Vec2 &a, Vec2 &b )
	{
		Vec2 dir = segEnd - segStart;
		float length = dir.length();
		dir /= length;
			
		a = Vec2( halfWidth * dir.y, halfWidth * -dir.x );
		b.x = -a.x;
		b.y = -a.y;
...
void TentacleRenderer::_decompose()
{
	...
	for( spline_rep_vec::const_iterator 
		...
		++splineRep )
	{
		const std::vector< Vec2 > &spline = splineRep->splineVertices;
			...
			++s0, ++s1 )
		{
			...
			_quads.push_back(vertex(a, Vec2( distanceAlongTentacle, ty ), heat, opacity ));
			_quads.push_back(vertex(b, Vec2( nextDistanceAlongTentacle, ty ), heat + heatIncrement, opacity + opacityIncrement ));
			_quads.push_back(vertex(c, Vec2( nextDistanceAlongTentacle, ty2 ), heat + heatIncrement, opacity + opacityIncrement ));
			_quads.push_back(vertex(d, Vec2( distanceAlongTentacle, ty2 ), heat, opacity ));
