...
void PowerPlatform::update( const core::time_state &time )
{
	...
	if ( _currentPowerCell )
	{
		...
		{			
			...
			const Vec2 
				CurrentPosition = v2( cpBodyGetPos( powerupBody )), 
				InitialTargetPosition = position() + Vec2( 0, _initializer.size.y * 0.5 + _currentPowerCell->initializer().size * 0.5 ),
				InjestedTargetPosition = position() + Vec2( 0, _initializer.size.y * 0.5 - _currentPowerCell->initializer().size );
			...
			Vec2 targetPosition = InitialTargetPosition;
...
void PowerPlatformRenderer::_drawGame( const core::render_state &state )
{
	...
	if ( !_svg )
	{
		...
		Vec2 scale = platform->initializer().size / documentSize;
		_svg.setScale( scale );
