...
void SensorRenderer::_drawDebug( const core::render_state &state )
{
	...
	const Vec2
		CenterWorld(v2(cpBBCenter(sensor->aabb()))),
		CenterScreen( state.viewport.worldToScreen(CenterWorld)),
		Center( state.viewport.screenToWorld( Vec2( std::floor( CenterScreen.x ), std::floor( CenterScreen.y))));
