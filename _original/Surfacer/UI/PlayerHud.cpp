...
namespace {
	...
	Surface8u CreateRotaryDialRing( int outerRadius, int innerRadius )
	{
		...
		while( iterator.line() )
		{
			...
			{
				const Vec2 Position( iterator.getPos() - Center );
										
				real sum = 0;
				for ( int i = 0; i < NumSamples; i++ )
				{
					const Vec2 Sample = Position + Samples[i];
				...
				const Vec2 Dir( rotateCCW(normalize( Position )));
...
void PlayerHud::resize( const Vec2i &newSize )
{
	if ( _health && _power )
	{
		...
		_weaponList->setPosition( PowerPosition + Vec2( Radius + Pad, -MHeight(_weaponList->initializer().font)/2 ));
	}
}
...
void PlayerHud::update( const time_state &time )
{
	...
	if ( FrazzleStrength > ALPHA_EPSILON && (!(time.step % 2)))
	{
		...
		_injuryEffectFilterHud->setRedChannelOffset( Rand::randFloat( OffsetRange ) * Vec2( Rand::randFloat(-1,1), 0 ));
		_injuryEffectFilterHud->setGreenChannelOffset( Rand::randFloat( OffsetRange/16 ) * Vec2( Rand::randFloat(-1,1), 0 ));
		_injuryEffectFilterHud->setBlueChannelOffset( Rand::randFloat( OffsetRange/4 ) * Vec2( Rand::randFloat(-1,1), 0 ));
	}
}
...
void ImageView::_layout()
{
	...
	switch( _initializer.fit )
	{
		case FIT:
		{
			...
			const Vec2 Range( Size - scaledImageSize );
		...
		case NATURAL:
		{
			const Vec2 Range( Size - ImageSize );
...
void RotaryDial::initialize( const init &initializer )
{
	...
	if ( _icon )
	{
		const Vec2 IconSize = _icon.getSize();
...
{
	const Vec2 Center( v2( cpBBCenter(bounds())));
...
void RotaryDial::_renderLabel( display_state &state )
{
	...
	state.labelSize = Vec2( _labelTexture.getSize().x, _labelFont.getAscent());
}
...
void SelectionList::draw( const core::render_state &state )
{
	...
	glBlendFuncSeparate( GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA, GL_ONE, GL_ONE );
	gl::draw( _compositedListTexture, _position + Vec2(0,yOffset));
	
	if (state.mode == RenderMode::DEBUG )
	{
		gl::color(1,0,1,0.5);
		gl::drawLine( _position, _position + Vec2( std::max( _compositedListTexture.getWidth(), 10 ), 0 ));
	}
...
void SelectionList::setPosition( const Vec2 &position )
{
	_position = position;
}

Vec2 SelectionList::position() const
{
	return _position;
}
...
void MagnetoBeamQueryOverlay::draw( const core::render_state &state )
{
	...
	if ( beam->active() )
	{
		...
		ColorA color;
		Vec2 screenPos;
		const Vec2 ScreenSize = state.viewport.size();
		...
		if ( CurrentlyAimedAt ) 
		{
			if ( CurrentlyAimedAt.towable )
			{
				...
				screenPos = state.viewport.worldToScreen( v2( cpBodyGetPos(CurrentlyAimedAt.target.body)));
			}
			else
			{
				color = ColorA(1,0,0,0.2);
				screenPos = state.viewport.worldToScreen( v2( cpBodyGetPos(towBody)));
			}
		}
		...
		gl::drawLine( Vec2( screenPos.x, 0), Vec2(screenPos.x, ScreenSize.y));
		gl::drawLine( Vec2( 0, screenPos.y), Vec2( ScreenSize.x, screenPos.y ));
	}
}
	...
	if ( !dismissed() && _renderedText )
	{
		...
		if ( GameObject *target = _resolveTrackingTarget() )
		{
			...
			const Vec2
				...
				TagPositionScreen = Vec2( PositionScreen.x - _renderedText.getWidth()/2, PositionScreen.y + layer->padding() );
...
void RichTextBox::setPosition( const Vec2 &p )
{
	_position = p;
	_updateScreenPosition();
}
...
void RichTextBox::_updateScreenPosition()
{
	if ( _renderedText && _initializer.trackingTargetIdentifier.empty() )
	{
		...
		const Vec2
			Pad = Vec2( layer->padding(), layer->padding() ),
			LayerSize = layer->size() - 2*Pad,
			Range = LayerSize - Vec2( _renderedText.getWidth(), _renderedText.getHeight() ),
