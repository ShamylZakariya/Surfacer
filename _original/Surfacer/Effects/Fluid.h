...
class FluidRendererModel : public core::util::JsonInitializable
{
	...
	public:
		...
		virtual void setFluidHighlightColor( const Vec4 &hc ) { _highlightColor = hc; }
		Vec4 fluidHighlightColor() const { return _highlightColor; }
...
class Fluid : public core::GameObject, public FluidRendererModel
{
	public:
		...
		struct init : public FluidRendererModel::init
		{
			...
			Vec2 boundingVolumePosition;		// position of bounding circle to fill with particles, if boundingVolumeRadius > 0
	...
	public:
		...
		*/
		void emitCircularVolume( const particle_init &particleInitializer, const Vec2 &volumePosition, real volumeRadius );
