...
namespace {
	...
	const static Vec2 ParticleShape[4] = 
	{ 
		Vec2( -1, -1 ),
		Vec2( +1, -1 ),
		Vec2( +1, +1 ),
	...
	const static Vec2 AtlasOffset_None[1] = { Vec2(0,0) };

	const static Vec2 AtlasOffset_TwoByTwo[4] = 
	{ 
		Vec2( 0*r2, 1*r2), Vec2( 1*r2, 1*r2), 
		Vec2( 0*r2, 0*r2), Vec2( 1*r2, 0*r2)
	}; 

	const static Vec2 AtlasOffset_FourByFour[16] = 
	{ 
	...
	const static Vec2 const *AtlasOffsetsByAtlasType[3] = {
		AtlasOffset_None,
...
void ParticleSystemRenderer::_updateParticles()
{
	...
	const Vec2 const* AtlasOffsets = AtlasOffsetsByAtlasType[atlasType];
	...
	if ( system->controller()->rotatesParticles() )
	{
		...
		for ( std::vector< particle >::const_iterator particle( system->particles().begin()), pEnd( system->particles().begin() + system->activeParticleCount()); particle != pEnd; ++particle )
		{
			...
			for ( int i = 0; i < 4; i++ )
			{
				...
				ci::Vec4 pc = particle->color;
				vertex->color = ci::Vec4( pc.x * pc.w, pc.y * pc.w, pc.z * pc.w, pc.w * ( real(1) - particle->additivity ));				
...
{
	Vec2 gravity = v2( cpSpaceGetGravity( system()->level()->space() ));
		 ...
		 particle != end; ++particle )
	{
		...
		if ( state._alive )
		{
			...
			const Vec2 changeInPosition = state._linearVelocity * time.deltaT;
			...
			if ( state.orientsToVelocity )
			{
				if ( vel > real(0) )
				{
					Vec2 dir = state._linearVelocity / vel;
...
bool UniversalParticleSystemController::_emit( const particle_state &emitState, const Vec2 &position, const Vec2 &velocity )
{
...
/*
	...
	struct emission_state {
		...
		emission_state( const particle_state &s, const Vec2 &p, const Vec2 &v ):
			state( s ),
			position( p ),
			velocity( v ) 
		{}
...
void UniversalParticleSystemController::Emitter::emit( std::size_t count, const Vec2 &position, const Vec2 &velocity )
{
...
void UniversalParticleSystemController::Emitter::emit( const particle_state &particle, const Vec2 &position, const Vec2 &velocity )
{
	if ( _throttled )
	{
