...
struct particle {
	ci::Vec2	position;			// position in world coordinates
	ci::Vec4	color;				// color of particle
...
class UniversalParticleSystemController : public ParticleSystemController
{
	public:
		...
		struct particle_state 
		{
			...
			range<Vec4> color;				//	color of particle
			...
			/**
				...
				The default simply returns Vec2(0,0)
			*/
			std::function< Vec2() > initialVelocity;
			...
			Vec2 _linearVelocity;			//	linearVelocity of the particle. Used by Controller
			...
			particle_state():
				...
				additivity(0,0),
				angularVelocity(0,0),
				color(Vec4(1,1,1,1),Vec4(1,1,1,1)),
			...
			static Vec2 defaultInitialVelocity() { return Vec2(0,0); }
		};
		...
		class Emitter {
			...
			public:
				...
				*/
				void emit( std::size_t count, const Vec2 &position, const Vec2 &velocity = Vec2(0,0) );
				
				/**
					Emit one particle described by @a particle at a given position with a given initialVelocity
				*/
				void emit( const particle_state &particle, const Vec2 &position, const Vec2 &velocity = Vec2(0,0) );
	...
	public:
		...
		/**
			...
			This initialize() method corresponds to use of:
				void emit( std::size_t count, const Vec2 &position, const Vec2 &velocity, real mutation );
		*/
	...
	private:
		...
		*/
		bool _emit( const particle_state &particle, const Vec2 &position, const Vec2 &velocity );
