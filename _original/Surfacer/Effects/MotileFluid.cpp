...
void MotileFluid::_buildProtoplasmic()
{
	...
	for ( int i = 0, N = _initializer.numParticles; i < N; i++ )
	{
		...
		physicsParticle.offsetPosition = radius * Vec2( std::cos( offsetAngle ), std::sin( offsetAngle ));
...
void MotileFluid::_updateProtoplasmic( const time_state &time )
{
	...
	const Vec2 
		mainBodyPosition = v2( cpBodyGetPos( _centralBody )),
		up( 0,1 );
		...
		++physicsParticle, ++fluidParticle )		
	{
		...
		fluidParticle->position = v2(cpBodyGetPos( physicsParticle->body ));
		...
		Vec2 dir = normalize( fluidParticle->position - mainBodyPosition );
...
void MotileFluid::_buildAmorphous()
{
	...
	for ( int i = 0, N = _initializer.numParticles; i < N; i++ )
	{
		...
		physicsParticle.scale = 1;
		physicsParticle.offsetPosition = overallRadius * Vec2( std::cos( offsetAngle ), std::sin( offsetAngle ));
	...
	if ( true )
	{
		...
		centralParticle.position = v2( cpBodyGetPos( _centralBody ));
