...
namespace {
	...
	Vec2 upscaleIfNecessary( Vec2 size, const Vec2 &targetSize )
	{
...
{
	for ( int i = 0; i < 4; i++ ) _vertices[i] = Vec3(0,0,0);
}
...
void BackgroundRenderer::draw( const render_state &state )
{
	...
	Vec2 bottomLeft = vp.screenToWorld( Vec2(0,0)),
		  topRight = vp.screenToWorld( Vec2( vp.size()) );
...
void BackgroundRenderer::_drawParallaxTexturedBackground( const render_state &state, const Rectf &viewportWorld )
{
	...
	Vec2 
		...
		normalizedTextureOffset( dir * 0.5 + Vec2(0.5,0.5));
...
//		normalizedTextureOffset( frustum.l / window.x, frustum.b / window.y ); 
	...
	{
		Vec2 size = layers[i].scale * vpSize;
...
void BackgroundRenderer::_drawSectorOverlay( const render_state &state, const Rectf &viewportWorld )
{
	...
	for ( int col = colStart; col <= colEnd; col += sectorSize )
	{
		if ( col == 0 )
		{
			...
			gl::drawLine( Vec2( real(col), 0 ), Vec2( real(col), top ));
			gl::color( ColorA(0.25,0,0,0.75));
			gl::drawLine( Vec2( real(col), 0 ), Vec2( real(col), bottom ));
		}
		else
		{
			gl::color( ColorA(1,1,1,0.75));
			gl::drawLine( Vec2( real(col), bottom ), Vec2( real(col), top ));
		}
	}	
	...
	for ( int row = rowStart; row >= rowEnd; row -= sectorSize )
	{
		if ( row == 0 )
		{
			...
			gl::drawLine( Vec2( 0, real(row) ), Vec2( right, real(row) ));
			gl::color( ColorA(0,0.25,0,0.75));
			gl::drawLine( Vec2( 0, real(row) ), Vec2( left, real(row) ));
		}
		else
		{
			gl::color( ColorA(1,1,1,0.75));
			gl::drawLine( Vec2( left, real(row) ), Vec2( right, real(row) ));
		}
	}	
