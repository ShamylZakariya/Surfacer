...
class ImageView : public core::ui::View
{
	public:
		...
		struct init {
			ci::gl::Texture image;
			fit_type fit;
			Vec2r alignment;	/* goes from 0 to 1 in x and y to position image, with 0.5,0.5 being centered */
			ci::ColorA color;
	...
	public:
		...
		void setAlignment( const Vec2r &align ) { _initializer.alignment = saturate( align ); _invalidateLayout(); }
		Vec2r alignment() const { return _initializer.alignment; }
...
class RotaryDial : public core::ui::View
{
	...
	protected:
		...
		struct display_state {
			...
			int whole;
			real fractional;
			Vec2r iconSize, labelSize, iconPosition, labelPosition;
			ci::Rectf iconRect;
			real iconScale;
...
class RichTextBox : public core::ui::View
{
	...
	public:
		...
		*/
		void setPosition( const Vec2r &p );
