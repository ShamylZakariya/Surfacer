...
void Monster::playerContactAttack( const Vec2r &positionWorld, real injuryScale )
{
	...
	switch ( state.mode ) 
	{
		case RenderMode::DEBUG:
		{
			if ( state.time - _lastPlayerVisibleTime < shortTermMemory() )
			{
				...
				Vec2r pp = lastKnownPlayerPosition();
...
Vec2r MonsterController::_directionToPlayer() const
{
	return normalize(lastKnownPlayerPosition() - monster()->position());
}
		
bool MonsterController::_findPlayer( Player *player, Vec2r &playerPosition ) const
{
...
void GroundBasedMonsterController::draw( const render_state &state )
{
	...
	switch ( state.mode ) 
	{
		case RenderMode::DEBUG:
		{
			...
			gl::drawLine( v2r(leftOrigin), v2r(leftEnd));

			gl::color( _canMoveRight ? GoColor : NoGoColor );
			gl::drawLine( v2r(rightOrigin), v2r(rightEnd));	
