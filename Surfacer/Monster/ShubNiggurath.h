...
class Tentacle : public core::GameObject
{
	...
	public:
		...
		*/
		...
		void setColor( const Vec4f &c ) { _color = c; }
		Vec4f color() const { return _color; }	
...
class ShubNiggurath : public MotileFluidMonster
{
	...
	protected:
		...
		bool _aim( const Vec2r &targetPosition, real &linearVel, Vec2r &dir, int triesRemaining );
...
class TentacleRenderer : public core::DrawComponent
{
	...
	private:
		...
		struct vertex {
			...
			vertex( const ci::Vec2f &p, const ci::Vec2f &tc, float h, float o ):
				position(p),
				texCoord(tc),
				heat(h),
				opacity(o)
		...
		struct spline_rep {
			std::vector< ci::Vec2f > segmentVertices, splineVertices;
