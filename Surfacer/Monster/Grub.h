...
class Grub : public Monster
{
	...
	public:
		...
		virtual Vec2r position() const { return v2r(cpBodyGetPos( _headBody )); }
		virtual Vec2r up() const;
		virtual Vec2r eyePosition() const { return position(); }
...
class GrubRenderer : public core::DrawComponent
{
	...
	private:
		...
		struct vertex {
			...
			vertex( const ci::Vec2f &p, const ci::Vec2f &tc ):
				position(p),
				texCoord(tc)
			{}
		};
