...
void Centipede::addedToLevel( Level *level )
{
	...
	if ( true )
	{
		...
		if ( true )
		{
			...
			{
				_whiskers->addFrond( frondInit, *body, v2r(cpBodyGetPos(*body)) + Right, Right );
				_whiskers->addFrond( frondInit, *body, v2r(cpBodyGetPos(*body)) - Right, -Right );
			}
		}
		...
		if ( true )
		{
			...
			for ( int i = 0, N = 16; i < N; i++ )
			{
				...
				_whiskers->addFrond( frondInit, _headBody, v2r(cpBodyGetPos(_headBody)) + HeadDir * 0.1, HeadDir );
				_whiskers->addFrond( frondInit, _tailBody, v2r(cpBodyGetPos(_tailBody)) + TailDir * 0.1, TailDir );					
			}
		}
	}
}
...
Vec2r Centipede::up() const 
{ 
	Vec2r right = v2r( cpBodyGetRot( _headBody ));
	Vec2r up = rotateCCW(right);
	
	if ( up.y < 0 ) up *= -1;
	return up;
}

Vec2r Centipede::eyePosition() const
{
...
void CentipedeRenderer::_tesselate( const render_state &state )
{
	...
	*/
	...
	for ( ; circle != circlesEnd; ++circle, ++body, ++i )
	{
		...
		{
			// front tip
			_triangleStrip.push_back( vertex( pos + (up * width), ci::Vec2f(0.5, ty)));
		}

		{
			// somewhere in the middle
			_triangleStrip.push_back( vertex( pos + (right * width), ci::Vec2f( 1, ty + tyInc_2 )));
			_triangleStrip.push_back( vertex( pos - (right * width), ci::Vec2f( 0, ty + tyInc_2 )));
		}

		if ( i == iLast )
		{
			// end tip
			_triangleStrip.push_back( vertex( pos - (up * width), ci::Vec2f( 0.5, ty + tyInc )));
		}
...
void CentipedeRenderer::_drawHeadAndTail( const core::render_state &state )
{
	...
	if ( !_svgShader )
	{
		...
		Vec2r documentSize = _head.documentSize();
	...
	const Vec2r
		HeadPosition = v2r(cpBodyGetPos( headBody )) + rotateCCW(v2r(cpBodyGetRot( headBody ))) * HeadOffset,
		TailPosition = v2r(cpBodyGetPos( tailBody )) + rotateCW(v2r(cpBodyGetRot( tailBody ))) * HeadOffset;
