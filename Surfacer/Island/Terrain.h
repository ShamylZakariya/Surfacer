...
struct triangle 
{
	struct vertex {
		...
		vertex( const Vec2r &p, const Vec2r &tc ):
			position(p),
			texCoord(tc)
		{}
	};
	...
	inline
	triangle( const Vec2r &A, const Vec2r &tcA,
			  const Vec2r &B, const Vec2r &tcB,
			  const Vec2r &C, const Vec2r &tcC ):
		a( A, tcA ),
		b( B, tcB ),
		c( C, tcC )
	{}
...
class Island : public core::GameObject
{
	public:
		...
		*/
		...
		/**
			...
			@a distThreshold
		*/
		Voxel *findVoxelClosestTo( const Vec2r &worldPosition, real distThreshold ) const;
		...
		*/
		bool triangulate( Vec2r const *ordinalToCentroidRelativeOffset = NULL );
	...
	private:
		...
		bool _triangulate( Vec2r const *ordinalToCentroidRelativeOffset );
		void _triangulatePerimeters( const std::vector< Vec2rVec > &ordinalSpacePerimeter, 
		                             const Vec2r &offset, 
		                             std::vector< triangle > &triangulation );

		void _createPerimeterGreebling( Vec2r const *ordinalToCentroidRelativeOffset );
...
class IslandGroup : public core::GameObject
{
	...
	protected:
		...
		Mat4r _modelview, _modelviewInverse;
...
class Terrain : public core::GameObject, public core::BatchDrawDelegate
{
	...
	protected:
		...
		*/
		...
		/**
			Up-scale Vec2r to Vec2i for storage in std::set
		*/
		static inline Vec2i _upscalePosition( const Vec2r &position )
		{
		...
		*/
		static inline Vec2r _downscalePosition( const Vec2i &upscaledPosition )
		{
			return Vec2r( upscaledPosition.x / real(32), upscaledPosition.y / real(32) );
		}
