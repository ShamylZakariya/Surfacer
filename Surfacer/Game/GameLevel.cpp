...
namespace {
	...
	Vec2r sparkInitialVelocity()
	{
		const real scale = 4;
		return Vec2r( Rand::randFloat(-2*scale, 2*scale), scale );
	}
...
void GameLevel::emitAcid( const Vec2r &position, const Vec2r &vel )
{
...
void GameLevel::emitAcidInCircularVolume( const Vec2r &position, real radius, const Vec2r &vel )
{
	Fluid::particle_init AcidParticle;
	AcidParticle.radius = 0.3;
	AcidParticle.density = 20;
...
void GameLevel::_terrainWasCut( const Vec2rVec &positions, TerrainCutType::cut_type cutType )
{
	...
	foreach( const Vec2r &position, positions )
	{
...
void GameLevel::_createEffectEmitters()
{
...

	{
		...
		sparkParticle.color = Vec4f( 1.0,0.6,1.0,1.0 );
		...
		fireParticle.color = Vec4f( 0.8,0.5,1.0,0.2 );
		...
		smokeParticle.color.min = Vec4f( 0.5,0.5,0.5,0.125 );
		smokeParticle.color.max = Vec4f( 0.2,0.2,0.2,0 );
	...
	
	{
		...
		const real Mutability = 0.25;
		const Vec3f TerrainColor( TerrainMaterialColor.r, TerrainMaterialColor.g, TerrainMaterialColor.b );
		UniversalParticleSystemController::init controllerInit;
		...
		smokeParticle.lifespan = 4;
		smokeParticle.color.min = Vec4f( TerrainColor,0.75 );
		smokeParticle.color.max = Vec4f( TerrainColor,0.00 );		
		...
		rubbleParticle.color = Vec4f( TerrainColor,1 );
...

	{
		...
		fireParticle.color.min = Vec4f( 1.0,0.9,0.7,0.25 );
		fireParticle.color.max = Vec4f( 1.0,0.2,0.0,0.5 );
		...
		smokeParticle.color.min = Vec4f( 0.1,0.1,0.1,0.2 );
		smokeParticle.color.max = Vec4f( 0.4,0.4,0.4,0.0  );
		smokeParticle.angularVelocity = M_PI / 8;
		smokeParticle.atlasIdx = 0;
...

	{
		...
		smokeParticle.color.min = Vec4f( 0.5,0.5,0.5,0.5 );
		smokeParticle.color.max = Vec4f( 0.2,0.2,0.2,0 );
		smokeParticle.angularVelocity = M_PI / 8;
		smokeParticle.atlasIdx = 0;
		...
		smokeParticle.lifespan = 2;
		smokeParticle.color.min = Vec4f( 0.3,0.3,0.3,0.5 );
		smokeParticle.color.max = Vec4f( 0,0,0,0 );
...

	{
		...
		spatterParticle.color.min = Vec4f( 0.1,0.5,0.1,0.5 );
		spatterParticle.color.max = Vec4f( 0,0.25,0,0 );
		...
		spatterParticle.color.min = Vec4f( 0,0.35,0,0.75 );
		spatterParticle.color.max = Vec4f( 0,0.125,0,0 );
