...
void GameScenario::setup()
{
	...
	_injuryEffectFilter->setChannelOffsets(Vec2r(0,0), Vec2r(4,0), Vec2r(-4,0));
	_injuryEffectFilter->setScanlineStrength(0.25);
	_injuryEffectFilter->setColorOffset(Vec3r(0.5,0,0));
	_injuryEffectFilter->setColorScale(Vec3r(0.5,0.5,0.5));
...
void GameScenario::update( const core::time_state &time )
{
	...
	if ( player )
	{
		...
		if ( Strength > ALPHA_EPSILON && !(time.step % 3))
		{
			...
			_injuryEffectFilter->setChannelOffsets(Vec2r(0,0), Vec2r(GreenOffset,0), Vec2r(BlueOffset,0));
		}
	}
}
