...
#pragma mark - Projectile
	...
	void Projectile::processContacts() {
		...
		//
		//	Notify stage of contacts
		//

		const StageRef &stage = getStage();
		for (const auto &contact : _contacts) {
			stage->registerContactBetweenObjects(CollisionType::WEAPON, getObject(), cpShapeGetCollisionType(contact.shape), contact.object);
		}
	}
...
#pragma mark - BeamProjectile
	...
	void BeamProjectile::updateContacts() {
		...
		cpSpace *space = getStage()->getSpace()->getSpace();
...
#pragma mark - CutterProjectile
	...
	*/
	...
	void CutterProjectile::onReady(ObjectRef parent, StageRef stage) {
		...
		terrain::TerrainObjectRef terrain = static_pointer_cast<SurfacerStage>(stage)->getTerrain();
		if (terrain) {
...
#pragma mark - BeamProjectileDrawComponent
	...
	void BeamProjectileDrawComponent::onReady(ObjectRef parent, StageRef stage) {
...
#pragma mark - Gun
	...
	void Gun::firePulse() {
		...
		getStage()->addObject(Object::with("Pulse", { pulse, make_shared<BeamProjectileDrawComponent>() }));
	}
	...
	void Gun::fireCutter() {
		...
		getStage()->addObject(Object::with("Blast", { cutter, make_shared<BeamProjectileDrawComponent>() }));
	}
...
#pragma mark - JetpackUnicyclePlayerPhysicsComponent 
	...
	void JetpackUnicyclePlayerPhysicsComponent::onReady(ObjectRef parent, StageRef stage) {
		...
		_jetpackFuelStage = _jetpackFuelMax = config.jetpackFuelMax;
	...
	void JetpackUnicyclePlayerPhysicsComponent::step(const time_state &timeState) {
		...
		bool flying = isFlying() && _jetpackFuelStage > 0;
		...
		if (flying) {
			...
			_jetpackFuelStage -= config.jetpackFuelConsumptionPerSecond * timeState.deltaT;
		} else if (!isFlying()) {
			if (_jetpackFuelStage < _config.jetpackFuelMax) {
				_jetpackFuelStage += config.jetpackFuelRegenerationPerSecond * timeState.deltaT;
				_jetpackFuelStage = min(_jetpackFuelStage, _config.jetpackFuelMax);
			}
		}
	...
	double JetpackUnicyclePlayerPhysicsComponent::getJetpackFuelStage() const {
		return _jetpackFuelStage;
	}
...
#pragma mark - PlayerInputComponent
	...
	dvec2 PlayerInputComponent::getShootingTargetWorld() const {
		return getStage()->getViewport()->screenToWorld(getMousePosition());
	}
