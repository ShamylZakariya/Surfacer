...
{
	_gradient[0] = Vec4f( c.r, c.g, c.b, c.a );
}
...
{
	_gradient[1] = Vec4f( c.r, c.g, c.b, c.a );
}
...
void GaussianBlurFilter::_createKernel( int radius, std::vector< Vec2f > &kernel )
{
	kernel.clear();
	...
	for ( int i = -radius; i <= radius; i++ )
	{
		...
		kernel.push_back( Vec2f( i, std::sqrt(mag) ));
	}
...
void NoiseFilter::_render( ci::gl::Fbo input, const render_state &state )
{
	...
	_shader.uniform( "NoiseTexSize", ci::Vec2f( _noiseTexture.getSize() ));
	_shader.uniform( "NoiseTexOffset", ci::Vec2f( Rand::randInt( _noiseTexture.getWidth() ), Rand::randInt( _noiseTexture.getHeight())));
	_shader.uniform( "Strength", float(strength()));
...
void DamagedMonitorFilter::_render( ci::gl::Fbo input, const render_state &state )
{
	...
	_shader.uniform( "SampleSizes", static_cast<Vec3f>(_pixelSizesByChannel) );
	_shader.uniform( "RedOffset", static_cast<Vec2f>(_redChannelOffset) );
	_shader.uniform( "GreenOffset", static_cast<Vec2f>(_greenChannelOffset) );
	_shader.uniform( "BlueOffset", static_cast<Vec2f>(_blueChannelOffset) );
	_shader.uniform( "ScanlineStrength", static_cast<float>(_scanlineStrength) );
	_shader.uniform( "ColorScale", lrp( strength(), Vec4f(1,1,1,1), _colorScale ));
	_shader.uniform( "ColorOffset", lrp( strength(), Vec4f(0,0,0,0), _colorOffset ));
