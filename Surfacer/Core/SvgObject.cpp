...
namespace {
		...
		const TRANSFORMER &transformer )
	{
		...
		const std::vector<Vec2f> &vertices = sourceMesh.getVertices();
		const std::vector<size_t> &indices = sourceMesh.getIndices();
		
		std::vector<Vec2f> destVertices;
		destVertices.reserve(vertices.size());
		for( std::vector<Vec2f>::const_iterator v(vertices.begin()),end(vertices.end()); v!=end; ++v )
		{
		...
		for( SvgShape::strokevec::const_iterator s(sourceStrokes.begin()),end(sourceStrokes.end()); s != end; ++s )
		{
			...
			for( std::vector< Vec2f >::const_iterator v(s->vertices.begin()),vEnd(s->vertices.end()); v!=vEnd; ++v )
			{
	...
	struct to_world_transformer
	{
		...
		to_world_transformer( const Vec2f &dSize, real dScale, const Matrix44f tw ):
		...
		{
			Vec2f world = ( toWorld * v ) * documentScale;
	...
	{
		Vec2f localOrigin;
		to_local_transformer( const Vec2f &lo ):
			localOrigin(lo)
		{}
...
Rectf SvgShape::_projectToWorld( const Vec2r &documentSize, real documentScale, const Mat4r &worldTransform )
{
	Mat4r toWorld = worldTransform * _svgTransform;
...
void SvgShape::_makeLocal( const ci::Vec2f &originWorld )
{
...
void SvgShape::_drawDebug( const render_state &state, SvgObject *owner, real opacity, ci::TriMesh2d &mesh, strokevec &strokes )
{
	if ( origin() )
	{
		Mat4r worldTransform;
		owner->_worldTransform( worldTransform );
		
		real screenSize = 20;
		Vec2r xAxis( worldTransform.getColumn(0).xy()),
		      yAxis( worldTransform.getColumn(1).xy());
		
		Vec2r scale( screenSize / xAxis.length(), screenSize / yAxis.length());
		scale *= state.viewport.reciprocalZoom();

		gl::color(1,0,0);
		gl::drawLine( Vec2f(0,0), Vec2f(scale.x,0));
		gl::color(0.25,0,0);
		gl::drawLine( Vec2f(0,0), Vec2f(-scale.x,0));

		gl::color(0,1,0);
		gl::drawLine( Vec2f(0,0), Vec2f(0,scale.y));
		gl::color(0,0.25,0);
		gl::drawLine( Vec2f(0,0), Vec2f(0,-scale.y));
	}
	else
	{
		if ( filled() )
		{
			...
			const std::vector<Vec2f> &vertices( mesh.getVertices() );
		...
		{	
			...
			foreach( const stroke &s, strokes )
			{
				...
				for( std::vector< ci::Vec2f >::const_iterator v(s.vertices.begin()),end(s.vertices.end()); v!=end; ++v )
				{
					gl::vertex( *v );
				}
	...
	if ( stroked() )
	{
		...
		for( strokevec::iterator stroke(strokes.begin()),end(strokes.end()); stroke != end; ++stroke )
		{
			if ( !stroke->vbo )
			{
				...
				glBufferData( GL_ARRAY_BUFFER, 
							  stroke->vertices.size() * sizeof( Vec2f ), 
...
/*
			...
			Vec2r localTransformPosition, localTransformScale, documentSize;			
			Mat4r groupTransform, transform;
...
SvgObject::SvgObject( ci::DataSourceRef svgFile, real scale )
{
	...
	_normalize( _obj->documentSize, scale, Mat4r(), Vec2r(0,0) );
}
...
Vec2r SvgObject::documentSize() const
{
	return root()._obj->documentSize;
}

void SvgObject::setPosition( const Vec2r &position )
{
...
void SvgObject::setScale( const Vec2r &scale )
{
...
Vec2r SvgObject::localToGlobal( const Vec2r &p )
{
	Mat4r worldTransform;
...
Vec2r SvgObject::globalToLocal( const Vec2r &p )
{
	Mat4r worldTransform;
	_worldTransform( worldTransform );
...
void SvgObject::_updateTransform()
{
	if ( _obj->transformDirty )
	{
		...
		Mat4r 
			S = Mat4r::createScale( _obj->localTransformScale ),
			T = Mat4r::createTranslation( Vec3f( _obj->localTransformPosition )),
			R;
...
void SvgObject::_normalize( const Vec2r &documentSize, real documentScale, const Mat4r &worldTransform, const Vec2r &parentWorldOrigin )
{
	//
	//	Project shapes to world, and gather their collective bounds in world space
	//

	Mat4r toWorld = worldTransform * _obj->groupTransform;
	...
	Vec2r worldOrigin = worldBounds.getCenter();
	...
	_obj->groupTransform = Mat4r::createTranslation( Vec3r( worldOrigin - parentWorldOrigin, 0 ));
	...
	if ( isRoot() )
	{
		...
		_obj->groupTransform = Mat4r::identity();
	}
}

void SvgObject::_worldTransform( Mat4r &m )
{
