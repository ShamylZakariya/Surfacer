...
class SvgShape
{
	...
	private:
		...
		ci::Rectf _projectToWorld( const Vec2r &documentSize, real documentScale, const Mat4r &worldTransform );		
		void _makeLocal( const ci::Vec2f &originWorld );
		void _drawDebug( const render_state &state, SvgObject *owner, real opacity, ci::TriMesh2d &mesh, strokevec &strokes );
		void _drawGame( const render_state &state, SvgObject *owner, real opacity, ci::TriMesh2d &mesh, strokevec &strokes );
...
class SvgObject
{
	...
	public:
		...
		*/
		...
		Vec2r documentSize() const;
		
		void setPosition( const Vec2r &position );
		Vec2r position() const { return _obj->localTransformPosition; }
		
		void setAngle( real a );
		real angle() const { return _obj->localTransformAngle; }
		
		void setScale( real s ) { setScale( s,s ); }
		void setScale( real xScale, real yScale ) { setScale( Vec2r(xScale,yScale)); }
		void setScale( const Vec2r &scale );
		Vec2r scale() const { return _obj->localTransformScale; }
		...
		*/
		Vec2r localToGlobal( const Vec2r &p );

		/**
			Project a point in world space to this object's local space
		*/
		Vec2r globalToLocal( const Vec2r &p );
	...
	private:
		...
		*/
		void _normalize( const Vec2r &documentSize, real documentScale, const Mat4r &worldTransform, const Vec2r &parentWorldOrigin );
