...
Vec2r JsonToVec2r( const JsonTree &v )
{
	const JsonTree 
		X = v["x"],
		Y = v["y"];
		
	return Vec2r( X.getValue<real>(), Y.getValue<real>());
}

Vec3r JsonToVec3r( const JsonTree &v )
{
	const JsonTree 
		X = v["x"],
		Y = v["y"],
	...
	return Vec3r( X.getValue<real>(), Y.getValue<real>(), Z.getValue<real>());
}

Vec4r JsonToVec4r( const JsonTree &v )
{
	const JsonTree 
		X = v["x"],
		Y = v["y"],
	...
	return Vec4r( X.getValue<real>(), Y.getValue<real>(), Z.getValue<real>(), W.getValue<real>());
}
...
bool read( const JsonTree &value, const std::string &name, Vec2r &into )
{
	try
	{
		into = JsonToVec2r(value[name]);
		return true;
	}
	catch( const std::exception & )
	{}
...
bool read( const JsonTree &value, const std::string &name, Vec3r &into )
{
	try
	{
		into = JsonToVec3r(value[name]);
		return true;
	}
	catch( const std::exception & )
	{}
...
bool read( const JsonTree &value, const std::string &name, Vec4r &into )
{
	try
	{
		into = JsonToVec4r(value[name]);
		return true;
	}
	catch( const std::exception & )
	{}
