	...
	_onOffTransition(0)
{
	...
	{
		_normalizedCircle[i] = Vec2r( std::cos( r ), std::sin( r ));
		r += inc;
	}	
...
void CuttingBeamRenderer::addedToGameObject( GameObject * )
{
	...
	_colors[0] = Vec4r(init.beamColor);
	_colors[1] = Vec4r(init.beamHaloColor);
	_colors[2] = Vec4r(init.aimColor);
	_colors[3] = Vec4r(init.aimHaloColor);
}
...
void CuttingBeamRenderer::_renderCircle( const Vec2r &position, real radius, const Vec4r &color, vertex_vec::iterator &vertex ) const
{
	const Vec4r OuterColor( color.xyz(), color.w * 0.25 );
...
void CuttingBeamRenderer::_renderLine( 
	...
	real length, 
	real width, 
	const Vec4r &startColor, 
	const Vec4r &endColor, 
	vertex_vec::iterator &vertex ) const
{
	...
	*/
	...
	const Vec2r XOrigin(0,0), XEnd(length,0);
	...
	Vec2f 
		...
		b = R*Vec2r(0,0),
		c = R*Vec2r(0,-Radius),
		d = R*Vec2r(length,Radius),
		e = R*Vec2r(length,0),
		f = R*Vec2r(length,-Radius);
...
void CuttingBeamController::update( const time_state &time )
{
	...
	cuttingBeam->setDirection( range > 0 ? (dir/range) : Vec2r(1,0) );
}
