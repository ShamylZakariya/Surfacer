...
Vec2r PlayerPhysics::_groundSlope() const
{
...
//		Down = cpv( -Up * 5000 );
	...
	return normalize( rotateCW( v2r(normal) ));
}
...
void PogocyclePlayerPhysics::ready()
{
	...
	_positionOffset = v2r( cpvsub( cpBodyGetPos( _body ), cpBodyGetPos(_wheelBody)));
...
void PogocyclePlayerPhysics::update( const core::time_state &time )
{
...

	{
		...
		const Vec2r NewUp = (SlopeWeight * rotateCCW(_groundSlope)) + Vec2r(0, UpWeight ) + Vec2r( DirWeight * Dir,0);
...
Vec2r PogocyclePlayerPhysics::position() const
{
	return v2r(cpBodyGetPos( _wheelBody )) + _positionOffset;
}

Vec2r PogocyclePlayerPhysics::up() const
{
	return _up;
}

Vec2r PogocyclePlayerPhysics::groundSlope() const
{
	return _groundSlope;
}
