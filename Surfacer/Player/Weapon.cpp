...
BeamWeapon::_raycast( const Vec2r &start, const Vec2r &end, cpShape *ignore ) const
{
	...
	if ( query0.touchedShape || query1.touchedShape )
	{
		...
		if ( std::abs( query0.dist - query1.dist ) > 2 * Offset )
		{
			if ( query0.dist < query1.dist )
			{
				...
				target.normal = v2r(query0.normal);
			}
			else
			{
				target.position = start + Dir * query1.dist; 
				target.normal = v2r(query1.normal) ;
			}			
		}
		else
		{
			...
			target.normal = v2r(cpvnormalize( query0.normal + query1.normal));
		}
	}
