...
void MagnetoBeam::update( const time_state &time )
{
...

	{
		...
		const Vec2r	
			Dir = v2r(cpBodyGetPos(_towBody)) - player->position();
...
void MagnetoBeam::setTowPosition( const Vec2r &worldPos )
{
...
//	}
	...
	inline void TowabilityColorRamp( real towability, Vec4r &color )
	{
	...
	_beamShaderEdgeAttrLoc(0)
{
	...
	bs.noiseOffsetRate = Vec2r( 4,8 ) * NoiseOffsetRateScale;
	...
	bs.noiseScale = 3;
	bs.noiseOffset.set(0, 0);
	bs.noiseOffsetRate = Vec2r( 8,16 ) * NoiseOffsetRateScale;
	_beamStates.push_back( bs );
	...
	bs.noiseScale = 4;
	bs.noiseOffset.set(0, 0);
	bs.noiseOffsetRate = Vec2r( 16,32 ) * NoiseOffsetRateScale;
	_beamStates.push_back( bs );
	...
	switch( state.mode )
	{
		...
		case RenderMode::DEVELOPMENT:
		{
			...
			if ( CurrentlyTowing )
			{
				...
				const Vec2r Position = v2r( cpBBCenter(Bounds) );
...
void MagnetoBeamRenderer::_updateBeam( const render_state &state, beam_state &beamState )
{
	...
	Vec2r positionAlongBeam = beamState.start;
...
void MagnetoBeamRenderer::_drawBeam( const render_state &state, beam_state &beamState, ci::gl::GlslProg &shader )
{
	...
	shader.uniform( "Color", Vec4f( beamState.color.xyz(), beamState.color.w * beamState.transition ));

	{
		const GLsizei 
			Stride = sizeof( beam_vertex ),
			PositionOffset = 0,
			EdgeAttrOffset = sizeof( Vec2r );
