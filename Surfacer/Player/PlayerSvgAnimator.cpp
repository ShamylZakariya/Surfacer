...
void PlayerSvgAnimator::update( const time_state &time )
{
	...
	const Vec2r 
		CrouchOffset	= _crouch * -Vec2r( 0, PlayerHeight * 0.125 ) / Scale,
		TorsoBobOffset	= BobScale * _motion * Vec2r( 0, PlayerHeight * 0.0125 * BobPhase0 ) / Scale,
		HelmetBobOffset = BobScale * _motion * Vec2r( 0, PlayerHeight * 0.0250 * BobPhase1 ) / Scale,
		GunBobOffset	= BobScale * _motion * Vec2r( 0, PlayerHeight * 0.0125 * BobPhase2 ) / Scale,
		ArmBobOffset	= BobScale * _motion * Vec2r( 0, PlayerHeight * 0.0250 * BobPhase3 ) / Scale;
...
Vec2r PlayerSvgAnimator::barrelEndpointWorld()
{
	return _parts.barrelEndpoint.localToGlobal( Vec2r(0,0));
}
