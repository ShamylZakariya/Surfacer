...
namespace {
	...
	class KeyboardMonsterController : public core::Component
	{
		public:
			...
			virtual void update( const time_state &time )
			{
				...
				if ( controller )
				{
					Vec2r dir = controller->enabled() ? controller->direction() : Vec2r(0,0);
	...
	Monster *monster_factory( Level *level, int type, const Vec2r &worldPosition )
	{
		switch( type )
		{
			case BARNACLE:
...
void MonsterPlaygroundScenario::setup()
{
	...
	if ( PLAYER_ENABLED )
	{
		...
		// by the powerups
		//playerInit.position = Vec2r( 200,172 ) * LevelScale;

		//	by the lava
		//playerInit.position = Vec2r( 205.899, 87.8 );

		//	by the test tongue
		playerInit.position = Vec2r( 140, 94 );
	...
	{
		Monster *monster = monster_factory( level, MONSTER_TYPE, Vec2r(200,88) );
	...
	if ( MIRRORS_ENABLED )
	{
		...
		mirrorInit.position = Vec2r( 80, 100 );
		Mirror *mirror = new Mirror();
		mirror->initialize( mirrorInit );
		level->addObject( mirror );
		
		mirrorInit.position = Vec2r( 70,120 );
		mirror = new Mirror();
		mirror->initialize( mirrorInit );
		level->addObject( mirror );
...
void MonsterPlaygroundScenario::draw( const render_state &state )
{
	...
	if ( !_monsters.empty() )
	{
		...
		gl::drawString( msg.str(), Vec2r(10, app::getWindowHeight() - 40 ), Color(1,1,1), _font );
	}
}
...
bool MonsterPlaygroundScenario::mouseMove( const app::MouseEvent &event, const Vec2r &delta )
{
	_mouseWorld = level()->camera().screenToWorld( event.getPos() );
	return false;
}

bool MonsterPlaygroundScenario::mouseDrag( const app::MouseEvent &event, const Vec2r &delta )
{
	GameLevel *level = gameLevel();
