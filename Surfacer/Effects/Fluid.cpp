...
void FluidRendererModel::initialize( const init &i )
{
	...
	setFluidHighlightColor( Vec4f( i.fluidHighlightColor.r, i.fluidHighlightColor.g, i.fluidHighlightColor.b, i.fluidHighlightColor.a ));
...
void FluidRenderer::_drawGame( const render_state &state )
{
	...
	
	{
		...
		if ( _backgroundTexture )
		{
			...
			_compositingShader.uniform( "BackgroundImageSize", Vec2r(_backgroundTexture.getWidth(),_backgroundTexture.getHeight()) );
		}		
	}
...
void FluidRenderer::_drawParticles( const render_state &state )
{
	...
	const ci::Vec2f vertexTemplate[4] = {
		ci::Vec2f( -particleCorner, -particleCorner ),
		ci::Vec2f( +particleCorner, -particleCorner ),
		ci::Vec2f( +particleCorner, +particleCorner ),
		ci::Vec2f( -particleCorner, +particleCorner )
	};
...
void Fluid::update( const time_state &time )
{
	...
	for( physics_particle_vec::iterator end( _physicsParticles.end()); physicsParticle != end; ++fluidParticle, ++physicsParticle )
	{
		...
		fluidParticle->position = v2r( cpBodyGetPos( body ));
...
void Fluid::emitCircularVolume( const particle_init &particleInitializer, const Vec2r &volumePosition, real volumeRadius )
{
	//
